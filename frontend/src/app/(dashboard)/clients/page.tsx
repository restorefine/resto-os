  ...
  return (
    <div className="max-w-[1400px]">
      ...
      <div className="flex gap-3 mb-6">
        {[
          { label: "Active", count: clients.filter((c) => c.status === "active").length, color: "bg-green-100 text-green-700" },
          { label: "Paused", count: clients.filter((c) => c.status === "paused").length, color: "bg-yellow-100 text-yellow-700" },
          { label: "Churned", count: clients.filter((c) => c.status === "churned").length, color: "bg-red-100 text-red-700" },
        ].map((s) => (
